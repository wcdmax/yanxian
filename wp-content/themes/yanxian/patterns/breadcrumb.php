 ...
 * Keywords: breadcrumb, navigation, 面包屑, 导航
...
<section class="uk-section uk-padding-small uk-padding-remove-horizontal">
    <div class="uk-container uk-container-large">
        <nav aria-label="Breadcrumb" class="uk-padding-small uk-padding-remove-horizontal uk-border-rounded uk-background-default">
            <ul class="uk-breadcrumb yk-breadcrumb uk-margin-remove-bottom">
                <li><a title="首页" href="<?php echo home_url(); ?>"><span class="yk-text-primary" uk-icon="icon: home; ratio: 0.8"></span> 首页</a></li>
                <?php
                if (is_category()) :
                    // 分类页面的面包屑
                    ...
                    foreach ($ancestors as $ancestor) :
                        $ancestor_obj = get_term($ancestor, 'category');
                        ?>
                        <li>
                            <a title="<?php echo esc_attr($ancestor_obj->name); ?>" 
                               href="<?php echo esc_url(get_category_link($ancestor)); ?>">
                ...
                elseif (is_page()) :
                    ...
                    if ($post->post_parent) :
                        ...
                        foreach ($ancestors as $ancestor) :
                            ?>
                            <li>
                                <a title="<?php echo esc_attr(get_the_title($ancestor)); ?>" 
                                   href="<?php echo esc_url(get_permalink($ancestor)); ?>">
